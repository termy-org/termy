...
fn render_configuration_doc() -> String {
    ...
    output.push_str("Tmux integration is an add-on. Set `tmux_enabled = true` to enable tmux mode; tmux-only commands and keybinds require restart and stay unavailable while tmux mode is disabled.\n\n");
...
fn render_default_config_template() -> String {
    ...
    output.push_str(
        "# tmux-only keybind actions require `tmux_enabled = true` and app restart\n",
    );
...
fn tmux_action_note(action: CommandId) -> &'static str {
    if action.is_tmux_only() {
        " (tmux required; restart required)"
    } else {
        ""
    }
}
...
mod tests {
    ...
    fn keybindings_doc_marks_tmux_only_actions() {
        ...
        assert!(out.contains("split_pane_vertical` (tmux required; restart required)"));
        assert!(out.contains("toggle_command_palette`"));
    }

    #[test]
    fn configuration_doc_mentions_tmux_restart_requirements() {
        let out = render_configuration_doc();
        assert!(out.contains("Tmux integration is an add-on"));
        assert!(out.contains("require restart"));
    }
}
