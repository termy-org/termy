...
fn render_configuration_doc() -> String {
    ...
    output.push_str("Tmux integration is an add-on. Set `tmux_enabled = true` to start in tmux mode by default. You can attach/detach tmux sessions at runtime from the command palette and File menu; tmux-only commands require an active tmux session.\n\n");
...
mod tests {
    ...
    #[test]
    fn configuration_doc_mentions_runtime_tmux_attach_detach() {
        let out = render_configuration_doc();
        assert!(out.contains("Tmux integration is an add-on"));
        assert!(out.contains("attach/detach tmux sessions at runtime"));
    }
}
