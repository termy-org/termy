...
- `toggle_agent_sidebar` is currently unavailable on Windows. On macOS/Linux it only works when `agent_sidebar_enabled = true`; enable it in Settings > Tabs or in `~/.config/termy/config.txt`.
