...
Tmux integration is an add-on. Set `tmux_enabled = true` to enable tmux mode; tmux-only commands and keybinds require restart and stay unavailable while tmux mode is disabled.
