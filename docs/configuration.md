...
Tmux integration is an add-on. Set `tmux_enabled = true` to start in tmux mode by default. You can attach/detach tmux sessions at runtime from the command palette and File menu; tmux-only commands require an active tmux session.
